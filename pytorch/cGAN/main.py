...
from optim.fromage import Fromage
...
parser.add_argument('--levels', type=int, help='number of discrete levels, if using multiplicative weights')
parser.add_argument('--initial_lr', type=float, default=0.01, help='initial learning rate')
parser.add_argument('--decayfactor', type=int, help='amount to decay intmadam by')
...
log_dir = f'logs/paper/{args.optim}-{args.initial_lr}-seed{args.seed}-levels{args.levels}-{args.decayfactor}/'
...
if args.optim == 'sgd': 
    ...
    optG = signSGD(netG.parameters(), lr=args.initial_lr)
    optD = signSGD(netD.parameters(), lr=args.initial_lr)
elif args.optim == 'madam':
    optG = Madam(netG.parameters(), lr=args.initial_lr)
    optD = Madam(netD.parameters(), lr=args.initial_lr)
elif args.optim == 'intmadam':
    optG = IntegerMadam(netG.parameters(), base_lr=args.initial_lr/args.decayfactor, lr_factor=args.decayfactor, levels=args.levels)
    optD = IntegerMadam(netD.parameters(), base_lr=args.initial_lr/args.decayfactor, lr_factor=args.decayfactor, levels=args.levels)
...
for epoch in range(0, args.epochs):
    ...
    if epoch==100:
        print("decaying")
        if args.optim=="intmadam":
            for group in optG.param_groups: group['lr_factor'] = 1
            for group in optD.param_groups: group['lr_factor'] = 1
        else:
